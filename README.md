...
<p align="left">
  一名热衷于将前沿AI技术（特别是AIGC和RAG）与工程实践相结合的软件开发者。
  <br>
  我擅长<b>主导AI辅助下的全栈项目开发</b>，将复杂的想法从0到1高效构建为稳定、可靠的应用程序。
  <br>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <ul>
          <li><strong>主导架构设计与优化:</strong> 独立设计了核心RAG链路，并主导了性能优化工作。通过引入<code>CompletableFuture</code>并行处理，使知识库首次加载效率提升<b>300%</b>。</li>
          <li><strong>攻坚实时交互体验:</strong> 确定采用<code>WebSocket</code>技术方案，并负责对AI生成的流式响应代码进行调试、测试与集成，将首次响应时间从~8秒缩短至<b>1秒内</b>。</li>
          <li><strong>负责安全与部署:</strong> 设计了基于<code>Spring Security</code>和<code>JWT</code>的RBAC权限模型，并独立编写优化的多阶段<code>Dockerfile</code>，通过<code>Docker Compose</code>实现一键部署。</li>
        </ul>
    ...
    <tr>
      ...
      <td>
        ...
        <ul>
          <li><strong>设计E2E自动化流水线:</strong> 负责设计了“数据提取 → AI处理 → 静态网站生成”的全自动化流程，并监督各模块的实现与集成。</li>
          <li><strong>实践AI工程化:</strong> 通过精巧的提示词工程（Prompt Engineering），主导实现了非结构化文本的智能标签提取、摘要生成等功能。</li>
          <li><strong>保障流畅GUI体验:</strong> 识别出UI卡顿的性能瓶颈，确定采用<code>Qt</code>多线程机制 (<code>QThread</code>) 方案，并负责对所有耗时操作进行异步化重构。</li>
        </ul>
    ...
    <tr>
      ...
      <td>
        一个集成了**在线商城**与**AI服装设计**功能的微信小程序，旨在通过数字化手段推广和传承民族服饰文化。
        <ul>
          <li><strong>主导全流程开发:</strong> 作为项目负责人，独立完成了从需求分析、技术选型到核心功能开发与上线的完整闭环。</li>
          <li><strong>负责AI功能集成:</strong> 主导完成了<code>Stable Diffusion API</code>的调研与集成工作，将前沿AIGC技术成功落地为项目核心亮点。</li>
          <li><strong>梳理并实现核心业务:</strong> 负责对电商项目的复杂业务逻辑（如购物车、订单状态管理）进行梳理，并监督代码实现质量。</li>
        </ul>
